...
// KillSessionIDIfGenerationAndName kills id only while both its server
// generation and current name still match the identity captured by the UI.
// The combined format predicate and kill run in one tmux command queue
// operation, closing the rename-between-confirmation-and-kill race.
...
// RenameSessionIDIfGenerationAndName renames id only while both its server
// generation and current name still match the identity captured by the UI.
func (s *Server) RenameSessionIDIfGenerationAndName(id, generation, oldName, newName string) error {
	if !isSessionID(id) {
		return fmt.Errorf("invalid tmux session id %q", id)
	}
	if !isGeneration(generation) {
...
// EntryNameOption records the encoded entry-session name a session belongs to.
// It makes the legacy-name migration idempotent when one encoded name is also
// another entry's legacy name.
...
func (s *Server) Sessions() ([]SessionInfo, error) {
	out, err := s.Run("list-sessions", "-F", "#{session_name}\t#{session_activity}\t#{session_attached}\t#{window_bell_flag}\t#{"+BellOption+"}\t#{"+EntryNameOption+"}\t#{"+EntryPathOption+"}\t#{session_id}\t#{"+ServerGenerationOption+"}\t#{session_created}")
	if err != nil {
	...
	for _, line := range strings.Split(out, "\n") {
		...
		act, _ := strconv.ParseInt(f[1], 10, 64)
