...
func (m *Manager) validateEntryInfo(name, entryPath string, info tmux.SessionInfo) error {
	...
	if info.EntryName == name && info.EntryPathToken == tmux.EncodeEntryPath(entryPath) {
		return nil
	}
	return m.markEntrySession(name, entryPath, info.ID, info.Generation)
}
...
func (m *Manager) markEntrySession(name, entryPath, id, generation string) error {
	if id == "" {
	...
	if err := m.Sess.SetSessionOptionIfGeneration(id, generation, tmux.EntryNameOption, name); err != nil {
...
func (m *Manager) migrateLegacyEntrySessions(entries []string, paths map[string]string, topologyComplete bool) error {
	...
	type migrationPlan struct {
		old, newName, targetID, targetGeneration string
	}
	...
	for _, newName := range encodedOrder {
		...
		plans = append(plans, migrationPlan{old: newName, newName: newName, targetID: info.ID, targetGeneration: info.Generation})
	...
	for _, old := range legacyOrder {
			...
			if hasSelection && sel.Session == old {
				...
				if targetAlive && ownerCurrent && config.LegacySessionName(m.WS, sel.Entry) == old {
					...
					case "":
						...
						plans = append(plans, migrationPlan{old: newName, newName: newName, targetID: migrated.ID, targetGeneration: migrated.Generation})
					case newName:
					default:
		...
		if info.EntryName == "" {
			...
			if len(differentEncodedOwners) > 0 {
				...
				case old:
					...
					plans = append(plans, migrationPlan{old: old, newName: old, targetID: info.ID, targetGeneration: info.Generation})
					continue
				case newName:
		...
		plans = append(plans, migrationPlan{old: old, newName: newName, targetID: info.ID, targetGeneration: info.Generation})
	...
	for _, plan := range plans {
		...
		if err := m.Sess.SetSessionOptionIfGeneration(plan.targetID, plan.targetGeneration, tmux.EntryNameOption, plan.newName); err != nil {
...
func (m *Manager) killEntrySession(name, targetID, targetGeneration, successor string, requireSameName bool) error {
	...
	var killErr error
	if requireSameName {
		killErr = m.Sess.KillSessionIDIfGenerationAndName(targetID, targetGeneration, name)
	} else {
...
func (m *Manager) renameTerm(oldName, targetID, targetGeneration, label string) (string, error) {
	...
	if err := m.Sess.RenameSessionIDIfGenerationAndName(
		targetID, targetGeneration, oldName, newName,
	); err != nil {
...
func (m *Manager) showDiff(repoRoot, relPath string, staged, untracked bool) error {
	...
	if _, _, err := m.Sess.NewSessionIdentity(name, repoRoot, fullCmd, generation); err != nil {
