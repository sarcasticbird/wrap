...
func TestQuitDetachesFromTerms(t *testing.T) {
	...
	m := NewModel(b, Options{WS: "vb"})
	var mod tea.Model = m
	mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{{Name: "vb·term·1"}}})
	mod.Update(key("q"))
	if !b.detached {
	...
	mod2, _ = mod2.Update(rowsMsg{sessions: []tmux.SessionInfo{{Name: "vb·term·1"}}})
...
func TestKillRejectsNonScratchSessionsBeforeConfirmation(t *testing.T) {
	for _, name := range []string{"vb", "vb/repo", "vb·diff"} {
		t.Run(name, func(t *testing.T) {
			b := &fakeBackend{}
			m := NewModel(b, Options{WS: "vb"})
			var mod tea.Model = m
			mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{{
				ID: "$7", Generation: "generation", Name: name, Created: 100,
			}}})
...
func TestKillConfirm(t *testing.T) {
	...
	m := NewModel(b, Options{WS: "vb"})
	var mod tea.Model = m
	mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{{ID: "$7", Generation: "generation", Name: "vb·term·1"}}})
	mod, _ = mod.Update(key("x"))
	if len(b.killed) != 0 {
...
func TestKillConfirmSurvivesRowChurn(t *testing.T) {
	...
	var mod tea.Model = m
	mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{
		{ID: "$7", Generation: "generation", Name: "vb·term·a", Created: 100, Activity: 100},
		{ID: "$9", Generation: "generation", Name: "vb·term·b", Created: 200, Activity: 100},
	}})
	...
	mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{
		{ID: "$7", Generation: "generation", Name: "vb·term·a", Created: 100, Activity: 100},
		{ID: "$8", Generation: "generation", Name: "vb·term·aa", Created: 150, Activity: 100},
		{ID: "$10", Generation: "new-generation", Name: "vb·term·b", Created: 300, Activity: 100},
	}})
	sm = mod.(Model)
...
func TestRenameNonTermRowErrors(t *testing.T) {
	...
	sm := mod.(Model)
	if sm.renaming {
		t.Error("r on a non-·term· row should not enter rename mode")
	}
...
func TestMouseIgnoredDuringConfirm(t *testing.T) {
	...
	var mod tea.Model = m
	mod, _ = mod.Update(rowsMsg{sessions: []tmux.SessionInfo{
		{ID: "$7", Name: "vb·term·a", Activity: 100},
		{ID: "$8", Name: "vb·term·b", Activity: 100},
	}})
