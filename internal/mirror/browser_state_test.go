...
const tags = {status: 3, close: 5, revoked: 9, error: 11};
let poisoned = false;
let renders = 0;
let endings = 0;
const effects = {
  ...
  validateSessionList: (value) => value.sessions,
  render: () => { renders += 1; },
  ended: () => { endings += 1; },
  error: () => {},
};
...
function deliver(frame) {
  try {
    check(
      reducer.receiveMessage(state, target, frame, effects),
      "lifecycle frame was not handled",
    );
  } catch {
    poisoned = true;
  }
...
deliver({tag: tags.status, payload: {sessions: [second, third]}});
...
deliver({tag: tags.close, payload: []});
...
deliver({tag: tags.revoked, payload: second});
